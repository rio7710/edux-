...
아래는 **최소 적용**을 위한 스키마 초안입니다.  
실제 적용 전 프로젝트 요구에 맞게 조정하세요.

### 9-1. 프로젝트 조건 테이블 (예: ProgramContext)

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `id` | string | PK |
| `projectType` | string | 사업 유형 (`정부사업` 등) |
| `agencyType` | string | 기관 유형 |
| `agencyName` | string | 기관명 (가명 가능) |
| `region` | string | 지역 |
| `targetAudience` | string | 대상 |
| `deliveryMode` | string | 운영 방식 |
| `policyTag` | string[] | 정책/사업 키워드 |
| `budgetRange` | string | 예산 구간 (선택) |
| `durationRange` | string | 교육 기간 구간 (선택) |
| `createdAt` | datetime | 생성일 |

### 9-2. 추천 로그 테이블 (예: RecommendationLog)

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `id` | string | PK |
| `contextId` | string | ProgramContext FK |
| `entityType` | string | `course`/`template` |
| `entityId` | string | 추천 대상 ID |
| `rank` | number | 추천 순위 |
| `recommendedBy` | string | `system`/`manual` |
| `accepted` | boolean | 채택 여부 |
| `used` | boolean | 사용 여부 |
| `curriculumVariant` | string | 적용된 커리큘럼 버전 |
| `createdAt` | datetime | 추천 시점 |

### 9-3. 이벤트 로그 테이블 (예: EventLog)

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `id` | string | PK |
| `eventType` | string | `course.create` 등 |
| `actorId` | string | 가명 사용자 ID |
| `entityType` | string | 대상 타입 |
| `entityId` | string | 대상 ID |
| `meta` | json | 추가 메타 |
| `createdAt` | datetime | 발생 시점 |
