...
## 9) 내일 작업 예정 (2026-02-12 목요일)

### 원칙

- **UX 먼저 확정** 후, 확정된 UX를 기준으로 백엔드/DB를 구현한다.
- 즉, 화면 흐름과 권한 시나리오를 먼저 고정하고, 데이터 모델은 그 다음에 맞춘다.

### 작업 예정 범위

1. 그룹 관리 UX 초안 작성  
2. 그룹별 기능 권한 매트릭스 화면 설계  
3. 사용자-그룹 할당 UX 설계  
4. 저장 시나리오(초안 저장/확정 반영) 플로우 정리  
5. 확정안 기준으로 백엔드 API + DB 스키마 작업 분할

---

## 10) 검토 요청 (Claude)

내일 UX 확정 전에 아래 항목 검토 요청:

1. 그룹 권한 모델 제안  
   - `Role + Group Permission` 합성 방식이 적절한지
2. 최소 DB 모델 제안  
   - `Group`, `UserGroup`, `GroupPermission`(+ 필요 시 `PermissionCatalog`)
3. 권한 충돌 규칙  
   - 허용/거부 우선순위, 기본값(deny-by-default) 정책
4. 점진 적용 전략  
   - read-only 관찰 단계 → 강제 적용 단계 전환 방식
